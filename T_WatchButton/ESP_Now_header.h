...
void transmit(unsigned long _inputNumber) {
  ...
  for (int i = 0; i < 16; i++) {
    Serial.print(cleartext[i], HEX); Serial.print("/");
  }
  Serial.println();
  ...
  esp_err_t result = esp_now_send(peer_addr, (uint8_t *)&ciphertext, sizeof(savedCounter));
